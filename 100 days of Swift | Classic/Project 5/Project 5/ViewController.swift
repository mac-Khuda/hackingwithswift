...
//  Project 5
//
//  Created by Volodymyr Khuda on 20.01.2021.
//
...
class ViewController: UITableViewController {
    ...
    @objc func startGame() {
        let wordForTitle = allWords.randomElement()
        title = wordForTitle
        ...
        if isPossible(word: lowerAnswer) {
            if isOriginal(word: lowerAnswer) {
                if isReal(word: lowerAnswer) {
                    ...
                    showErrorMessage(errorTitle: "Word not recognized", errorMessage: "You just can't make them up, you know!")
                }
            } else {
